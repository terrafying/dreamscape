...
export default function DreamDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setError('')
      try {
        const res = await apiFetch(`/api/similar/${params.id}?limit=5`)
  ...
  const handleReact = async (emoji: string) => {
    ...
    try {
      const res = await apiFetch(`/api/dreams/${params.id}/react`, {
        method: 'POST',
  ...
  const handleInterpret = async () => {
    ...
    try {
      const res = await apiFetch(`/api/dreams/${params.id}/interpret`, {
        method: 'POST',
  ...
  const dreamData = dream.dream_data as { transcript?: string; extraction?: { symbols?: { name: string; salience: number; category: string; meaning: string }[]; emotions?: { name: string; intensity: number; valence: number }[]; themes?: { name: string; confidence: number }[]; narrative_arc?: string; lucidity?: number; interpretation?: string } }
  const symbols = dreamData?.extraction?.symbols ?? []
  const emotions = dreamData?.extraction?.emotions ?? []
  const themes = dreamData?.extraction?.themes ?? []
      ...
      {similar.length > 0 && (
        <div className="space-y-3">
          ...
          {similar.map((s: SharedDreamWithCounts & { similarity: number }) => {
            const sim = s as SharedDreamWithCounts & { similarity: number }
